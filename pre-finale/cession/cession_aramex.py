...
def att_fret():
    ...
    dossier = f"Cession_doc_Aramex{date_str}/{awb}"
    image_path = './data/Logo celero/celero_logo.png'
    ...
    ce.add_run("CELERO Madagascar").bold=True
    ce.alignment = WD_ALIGN_PARAGRAPH.RIGHT
    
    outputFile = os.path.join(dossier,f'Att_fret{awb}.docx')
    doc.save(outputFile)
#bon a delivrer 
def bon_a_delivrer():
    name, awb, pcs,weight, currency, lta = find_information()
    ...
    outputFile =os.path.join(dossier,f'bon_a_delivrer{awb}.docx') 
    doc.save(outputFile)
...
def ordre_transit():
    ...
    dat.add_run(f"Fait à Antananarivo le {date_st}.")
    dat.alignment = WD_ALIGN_PARAGRAPH.RIGHT
    outputFile = os.path.join(dossier,f'ordre_transit{date_str}.docx')
    doc.save(outputFile)

# Entête de l'application
Label(root, text="CELERO EXPRESS CESSION DOCUMENT ARAMEX", width=10, height=3, bg="white", fg='blue', font="arial 20 bold").pack(side=TOP, fill=X)
...
currency =DoubleVar()
Label(root,text='Currency Aramex',bg=background, foreground="white", font='arial 10').place(x=290,y=280)
currency_entry = Entry(root, textvariable= currency, width=30)
currency_entry.place(x=250, y=310)

# Bouton pour télécharger un fichier
Button(root, text="Télécharger Manifest", font='arial 12', width=30, command=get_manifest).place(x=200, y=350)
# Bouton pour préparer les documents
Button(root, text="Générer la cession doc", font='arial 12', width=30, command=print_doc).place(x=200, y=400)

# Bouton retour
Button(root, text="Retour", width=30, font='arial 12').place(x=200, y=450)
# Bouton quitter
Button(root, text="Quitter", width=30, font='arial 12', foreground="red", command=root.destroy).place(x=200, y=500)
